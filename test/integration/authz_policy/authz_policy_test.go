...
func TestAuthzPolicy(t *testing.T) {
	// Initializes the test using the 'examples/authz_policy' directory
	bpt := tft.NewTFBlueprintTest(t)

	bpt.DefineVerify(func(assert *assert.Assertions) {
		// Verify no drift happens after terraform apply.
		// This proves that Terraform successfully created the policy and the state is stable.
